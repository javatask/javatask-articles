...
In this part, we will join add Authentication to the Frontend using AWS Cognito as OpenID Server. Figure 1 shows what we are trying to achieve
...
* Get AWS Cognito JSON Web Token
...
> Note. In the production environment, you will use Route53 subdomains and domains to configure all URLs.
...
We will configure new Frontend assets to use AWS Cognito Hosted UI. Please use the code from the `tutorial/part4/ui` folder \[TODO add link\].
...
Inside `tutorial/part4/ui` folder create `settings.js` file with the next content, you can use value from the Output section of CFN template used to deploy AWS Cognito:
...
Then install libraries by running in the `tutorial/part4/ui` folder
...
> Note. To update your Frontend assets you need not only to upload them to the S3 bucket, but also tell Cloudfront to invalidate Cache. You can invalidate the cache using `aws cloudfront create-invalidation --distribution-id <YOUR_ID> --paths "*"` command

## Get Authorization token

Click on the Login button, you should be redirected to the hosted AWS Congito page:
...
In the next Part, I'll show you how to connect all moving parts together.