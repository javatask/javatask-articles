...
Now we know why we need API Gateway, let's jump directly to our Cloudformation (CFN) templates.
...
I want to show the main two drawbacks of this configuration.

My EC2 Instance and API Gateway URL both has publically reachable URL. It's quite easy to fix by disabling Public IP and adding Authorizer to Api Gateway and putting it behind AWS Cloudfront.

But it's more complex to fix questions on upgrading your backend implementation - there is no easy way apart from hiding EC2 behind Load Balancer and building new AMI images with the latest version of your Backend container.
